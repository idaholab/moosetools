            ...
            try:
                ...
                for row in lines:
                    ...
                    if len(headers) != len(vals):
                        self.addError(
                            f, "Number of columns (" + str(len(vals)) +
                            ") not the same as number of column names (" + str(len(headers)) +
                            ") in row " + repr(row))
                        break
...
class CSVSummary(CSVTools):
    ...
    def summary(self):
        ...
        for field, value in table1.items():
            ...
            value_len.append(
                len("# min: %.3e @ t%d" % (Decimal(min(value)), value.index(min(value)))))
        ...
        for key in keys1:
            for val1, val2 in zip(table1[key], table2[key]):
                ...
                if rel_diff > rel_tol:
                    self.addError(
                        self.files[1],
                        "The values in column \"%s\" don't match @ t%d\n\trelative diff:   %.3e ~ %.3e = %.3e (%.3e)"
                        % (key.strip(), table2[key].index(val2), val1, val2, rel_diff,
                           Decimal(rel_diff)))
        ...
        if self.custom_columns and not self.__only_compare_custom:
            for mykey2 in self.custom_columns:
                if not found_column[mykey2]:
                    self.addError(
                        self.files[0],
                        "all CSV files Variable '" + mykey2 + "' in custom_columns is missing")
...
def verifyArgs(args):
    ...
    unify_custom_args = [
        x for x in [args.custom_columns, args.custom_abs_zero, args.custom_rel_err] if x != None
    ]
...
def parseArgs(args=None):
    ...
    parser.add_argument('--diff-fields',
                        '-f',
                        nargs='+',
                        metavar='id',
    ...
    parser.add_argument('--custom-columns',
                        nargs='+',
                        metavar='field',
    ...
    parser.add_argument(
        ...
        metavar='exponential',
        help=
        'Space separated list of corresponding exponential absolute zero values for --custom-colums'
    )
    parser.add_argument(
        '--custom-rel-err',
        nargs='+',
        metavar='exponential',
        help=
        'Space separated list of corresponding acceptable exponential tolerance values for --custom-colums'
    )
