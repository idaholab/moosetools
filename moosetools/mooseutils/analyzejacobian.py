...
class REStruct(object):
    def __init__(self, new_petsc):
        ...
        if not new_petsc:
            ...
            additional_string = ""
        self.MfdRE, self.MhcRE, self.MdiffRE = (re.compile(item + additional_string)
                                                for item in (self.MfdRE, self.MhcRE, self.MdiffRE))
        ...
        for j in range(nblocks):
            ...
            if norm[i][j] > rel_tol * fd[i][j] and norm[i][j] > abs_tol:
                ...
                if i == j:
                    ...
                    print("  (%d,%d) Off-diagonal Jacobian for variable '%s' %s" %
                          (i, j, nlvars[j], problem))
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        dest="method",
        help=
        "Pass either opt, dbg or devel.  Works the same as setting the $METHOD environment variable."
    )
    ...
    parser.add_option("-r",
                      ...
                      dest="resize_mesh",
                      action="store_true",
                      help="Perform resizing of generated meshs (to speed up the testing).")
    parser.add_option(
        "-s",
        "--mesh-size",
        dest="mesh_size",
    ...
    parser.add_option(
        ...
        dest="only",
        help=
        "Test specified Jacobians only (space separated list of comma separated variable pairs).")
    parser.add_option("-D",
                      "--on-diagonal-only",
                      dest="diagonal_only",
                      action="store_true",
                      help="Test on-diagonal Jacobians only.")

    parser.add_option("-d",
                      "--debug",
                      dest="debug",
                      action="store_true",
                      help="Output the command line used to run the application.")
    parser.add_option("-w",
                      "--write-matrices",
                      dest="write_matrices",
                      action="store_true",
                      help="Output the Jacobian matrices in gnuplot format.")
    parser.add_option(
        "-n",
        "--no-auto-options",
        dest="noauto",
        action="store_true",
        help=
        "Do not add automatic options to the invocation of the moose based application. Requres a specially prepared input file for debugging."
    )
    parser.add_option(
        "--rel-tol",
        dest="rel_tol",
        ...
        type="float",
        help=
        "The relative tolerance on the Jacobian elements between the hand-coded and those evaluated with finite difference."
    )
    parser.add_option(
        "--abs-tol",
        dest="abs_tol",
        ...
        type="float",
        help=
        "The absolute tolerance on the Jacobian elements between the hand-coded and those evaluated with finite difference."
    )
    parser.add_option(
        '--cli-args',
        type=str,
    ...
    if options.resize_mesh:
        moosebaseparams.extend([
            'Mesh/nx=%d' % options.mesh_size,
    ...
    if not options.noauto:
        ...
        mooseparams.extend([
            'Problem/solve=false', 'BCs/active=', 'Outputs/' + dofoutname + '/type=DOFMap',
            'Outputs/active=' + dofoutname, 'Outputs/file_base=' + basename + '_' + dofoutname,
            '--no-gdb-backtrace'
        ])
        ...
        print(
            "Running without automatic options DOF map '%s' will not be generated automatically!" %
            dofmapfilename)
    ...
    for var in dofdata['vars']:
        for subdomain in var['subdomains']:
            ...
            for dof in subdomain['dofs']:
                dofkernels[dof].extend(
                    [kernel['name'] for kernel in kernels if not kernel['name'] in dofkernels[dof]])
    ...
    #from testharness import util # circular import
    new_petsc = True  #list(map(int, util.getPetscVersion(libmesh_dir).split("."))) >= [3, 9]
    ...
    if not options.noauto:
        if new_petsc:
            petsc_test_options = [
                '-snes_test_jacobian', '-snes_test_jacobian_view', '-snes_type', 'ksponly',
        ...
        mooseparams.extend(petsc_test_options + [
            '-mat_fd_type', 'ds', 'BCs/active=', 'Outputs/exodus=false', 'Outputs/csv=false',
