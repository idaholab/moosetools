...
class TestHarnessTester(TestHarnessTestCase):
    ...
    def testWriteOK(self):
        ...
        self.runTests('--no-color', '-i', 'always_ok', '--sep-files-ok', '--output-dir',
                      self.output_dir)
        ...
        with self.assertRaises(subprocess.CalledProcessError):
            self.runTests('--no-color', '-i', 'diffs', '--sep-files-ok', '--output-dir',
                          self.output_dir)
        ...
        if (os.path.exists(os.path.join(self.output_dir, 'test_harness.exodiff.DIFF.txt'))
            ...
            self.fail(
                'Test results which failed were created when asked NOT to do so: --sep-files-ok')

    def testWriteFail(self):
        """ Test ability to write separate Fail test --sep-files-fail """
        with self.assertRaises(subprocess.CalledProcessError):
            self.runTests('--no-color', '-i', 'diffs', '--sep-files-fail', '--output-dir',
                          self.output_dir)
        ...
        self.runTests('--no-color', '-i', 'always_ok', '--sep-files-fail', '--output-dir',
                      self.output_dir)
        if os.path.exists(os.path.join(self.output_dir, 'test_harness.always_ok.OK.txt')):
            self.fail(
                'Test results which passed were created when asked NOT to do so: --sep-files-fail')

    def testWriteAll(self):
        """ Test write all output files --sep-files """
        with self.assertRaises(subprocess.CalledProcessError):
            self.runTests('--no-color', '-i', 'diffs', '--sep-files', '--output-dir',
                          self.output_dir)

        self.runTests('--no-color', '-i', 'always_ok', '--sep-files', '--output-dir',
                      self.output_dir)
