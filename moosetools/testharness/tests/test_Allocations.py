...
class TestHarnessTester(TestHarnessTestCase):
    def testSkippedAllocations(self):
        ...
        self.assertRegex(output.decode('utf-8'),
                         'tests/test_harness.always_ok.*? \[INSUFFICIENT SLOTS\] SKIP')
        ...
        output = self.runTests('--no-color', '-i', 'allocation_test', '--n-threads', '3', '-p', '3',
                               '-j', '8')
        self.assertRegex(output.decode('utf-8'),
                         'tests/test_harness.allocation_test.*? \[INSUFFICIENT SLOTS\] SKIP')
    ...
    def testCpuCaveats(self):
        ...
        output = self.runTests('-i', 'allocation_test', '-p', '4', '--n-threads',
                               '2').decode('utf-8')
        self.assertNotIn('MIN_THREADS', output)
    ...
    def testThreadCaveats(self):
        ...
        output = self.runTests('-i', 'allocation_test', '-j', '4', '-p', '2', '--n-threads',
                               '1').decode('utf-8')
        self.assertNotIn('CPUS', output)
        ...
        output = self.runTests('-i', 'allocation_test', '-j', '6', '-p', '2', '--n-threads',
                               '4').decode('utf-8')
        self.assertNotIn('CPUS', output)
        self.assertNotIn('OVERSIZED', output)
