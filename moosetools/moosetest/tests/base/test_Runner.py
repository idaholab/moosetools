...
class TestRunner(unittest.TestCase):
    ...
    def test_preExecute(self):
        runner = moosetest.base.Runner(name='run', filenames=('/runner_0', 'runner_1'))
        ...
        runner = moosetest.base.Runner(name='run', filenames=(os.path.abspath(__file__), ))
        with self.assertLogs(level='ERROR') as log:
            runner.preExecute()
        self.assertEqual(len(log.output), 1)
        self.assertIn("The following file(s) are being tracked with 'git'", log.output[0])

        runner = moosetest.base.Runner(name='run', filenames=('/file.e', ))
        ...
        runner = moosetest.base.Runner(name='run', check_created_files=True)
        with self.assertLogs(level='ERROR') as log:
            runner.preExecute()
        self.assertEqual(len(log.output), 1)
    ...
    def test_getExpectedFiles(self):
        d0 = moosetest.base.make_differ(moosetest.base.Differ, name='a', filenames=('/differ_a', ))
        d1 = moosetest.base.make_differ(moosetest.base.Differ, name='b', filenames=('differ_b', ))
        runner = moosetest.base.Runner(name='run',
                                       differs=(d0, d1),
        ...
        with mock.patch('os.path.isdir', return_value=True):
            ...
            d0.parameters().setValue('base_dir', '/base')
