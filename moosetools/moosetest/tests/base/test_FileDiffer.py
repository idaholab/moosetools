...
class TestFileDiff(unittest.TestCase):
    def testGoldFilenames(self):
        obj = FileDiffer(name='diff', filenames=('a', 'b'), gold_filenames=('c', 'd'))
        obj.preExecute()
        pairs = list(obj.pairs())
        self.assertEqual(pairs, [('a', 'c'), ('b', 'd')])

    def testGoldFilenamesError(self):
        obj = FileDiffer(name='diff', filenames=('a', 'b'), gold_filenames=('c', 'd', 'e'))
    ...
    def testGoldDir(self):
        obj = FileDiffer(name='diff', filenames=('a', 'b'))
        obj.preExecute()
        pairs = list(obj.pairs())
