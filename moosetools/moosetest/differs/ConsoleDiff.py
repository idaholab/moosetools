...
class ConsoleDiff(Differ):
    ...
    def validParams():
        ...
        params.add('text_in', vtype=str, doc="Checks that the supplied text exists in sys.stdout or sys.stderr.")
        params.add('text_not_in', vtype=str, doc="Checks that the supplied text does not exist in sys.stdout and sys.stderr.")
        ...
        params.add('re_match', vtype=str, doc="Checks that the supplied regular expression returns a match in sys.stdout or sys.stderr.")
        params.add('re_match_stdout', vtype=str, doc="Checks that the supplied regular expression returns a match in sys.stdout.")
        params.add('re_match_stderr', vtype=str, doc="Checks that the supplied regular expression returns a match in sys.stderr.")
        params.add('re_flags', vtype=str, array=True, default=('MULTILINE', 'DOTALL', 'UNICODE'),
                   allow=('MULTILINE', 'DOTALL', 'UNICODE', 'IGNORECASE', 'VERBOSE', 'LOCALE', 'DEBUG', 'ASCII'),
    ...
    def execute(self, rcode, stdout, stderr):
        ...
        if re_match is not None:
            match = re.match(re_match, stdout, flags=flags) or re.match(re_match, stderr, flags=flags)
            if not match:
